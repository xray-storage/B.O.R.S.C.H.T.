...
void EImageThumbnail::VFlip()
{
    ...
    for (int y=0; y<THUMB_HEIGHT/2; y++,y2--){
		CopyMemory(line,&m_Pixels.front()+y2*THUMB_WIDTH,sz_ln);
		CopyMemory(&m_Pixels.front()+y2*THUMB_WIDTH,&*m_Pixels.begin()+y*THUMB_WIDTH,sz_ln);
		CopyMemory(&m_Pixels.front()+y*THUMB_WIDTH,line,sz_ln);
    }
}
...
//	imf_filter	imf_box  imf_triangle  imf_bell  imf_b_spline  imf_lanczos3  imf_mitchell
	...
	imf_Process(&m_Pixels.front(),THUMB_WIDTH,THUMB_HEIGHT,p,w,h,imf_box);
}
