...
resplit:
		...
		if ((int(s1.size())<c_SS_LowVertLimit) || (int(s2.size())<c_SS_LowVertLimit))
		{
			...
			if (defl_base)	
			{
				...
				CDeflector*		D1	= xr_new<CDeflector>(); D1->OA_Place(s1); D1->OA_Export(); lc_global_data()->g_deflectors().push_back(D1);
				CDeflector*		D2	= xr_new<CDeflector>(); D2->OA_Place(s2); D2->OA_Export(); lc_global_data()->g_deflectors().push_back(D2);
			}
