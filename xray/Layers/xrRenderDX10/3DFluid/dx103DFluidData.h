...
public:
	...
	void	SetTexture(eVolumePrivateRT id, ID3D11Texture3D *pT) { pT->AddRef(); m_pRTTextures[id]->Release(); m_pRTTextures[id] = pT;}
	...
	ID3D11Texture3D*			GetTexture(eVolumePrivateRT id) const { m_pRTTextures[id]->AddRef(); return m_pRTTextures[id];}
...
private:
	...
	ID3D11Texture3D			*m_pRTTextures[ VP_NUM_TARGETS ];
};
