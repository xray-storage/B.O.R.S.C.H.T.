...
D3D11_CULL_MODE ConvertCullMode(D3DCULL Mode)
{
	switch (Mode)
	{
	case D3DCULL_NONE:
...
D3D11_COMPARISON_FUNC ConvertCmpFunction(D3DCMPFUNC Func)
{
	switch (Func)
	{
	case D3DCMP_NEVER:
...
D3D11_STENCIL_OP ConvertStencilOp(D3DSTENCILOP Op)
{
	switch (Op)
	{
...
D3D11_BLEND ConvertBlendArg(D3DBLEND Arg)
{
	switch (Arg)
	{
	case D3DBLEND_ZERO:
...
D3D11_BLEND_OP ConvertBlendOp(D3DBLENDOP Op)
{
	switch (Op)
	{
	case D3DBLENDOP_ADD:
...
D3D11_TEXTURE_ADDRESS_MODE	ConvertTextureAddressMode(D3DTEXTUREADDRESS Mode)
{
	switch(Mode)
	{
...
bool operator==(const D3D11_BLEND_DESC &desc1, const D3D11_BLEND_DESC &desc2)
{
	...
	int n = desc1.IndependentBlendEnable ? 8 : 1;
