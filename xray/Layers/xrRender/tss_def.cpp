...
void SimulatorStates::UpdateDesc( D3D11_BLEND_DESC &desc ) const
{
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==0)
		{
			...
			case D3DRS_SRCBLEND:
				desc.RenderTarget[0].SrcBlend = dx10StateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLEND:
				desc.RenderTarget[0].DestBlend = dx10StateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;
			
				//D3DRS_ALPHAFUNC

			case D3DRS_BLENDOP:
				desc.RenderTarget[0].BlendOp = dx10StateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;

			case D3DRS_SRCBLENDALPHA:
				desc.RenderTarget[0].SrcBlendAlpha = dx10StateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_DESTBLENDALPHA:
				desc.RenderTarget[0].DestBlendAlpha = dx10StateUtils::ConvertBlendArg((D3DBLEND)S.v2);
				break;

			case D3DRS_BLENDOPALPHA:
				desc.RenderTarget[0].BlendOpAlpha = dx10StateUtils::ConvertBlendOp((D3DBLENDOP)S.v2);
				break;

			case D3DRS_ALPHABLENDENABLE:
				desc.RenderTarget[0].BlendEnable = S.v2?1:0;
				break;
...
void SimulatorStates::UpdateDesc( D3D11_SAMPLER_DESC descArray[D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT], bool SamplerUsed[D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT], int iBaseSamplerIndex ) const
{
	...
	for (u32 it=0; it<States.size(); it++)
	{
		...
		if (S.type==2)
		{
			...
			if ( (iSamplerIndex>=D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT)
				|| iSamplerIndex<0)
				continue;
