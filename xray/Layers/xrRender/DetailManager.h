...
public:
	...
	void setMaskEditMode(MaskEditModes mode);// установить режим редактирования маски
	bool maskChanged() const;// маска изменена
	void saveMask();// сохранить маску

private:
	ref_texture m_mask;// Текстура маски
	u32* m_maskData = nullptr;// Пиксели маски в формате A8R8G8B8
	bool isMaskCleared(const Fvector& p) const;// Проверить по маске, что траву в точке p сажать не надо
	bool m_useMask = true;// Использовать маску
	MaskEditModes m_maskEditMode = MaskEditModes::None;//Режим редактирования маски
	bool m_maskChanged = false;// Маска изменена
